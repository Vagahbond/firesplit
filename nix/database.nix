{ pkgs }:
{
  pgconfigure = pkgs.writeShellScriptBin "pgconfigure" ''
    psql -h localhost -p 5432 -d postgres -c "CREATE USER firefly-iii WITH PASSWORD 'password';"
    psql -h localhost -p 5432 -d postgres -c "CREATE DATABASE firefly-iii WITH OWNER firefly-iii;";
  '';
